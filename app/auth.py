...
from .parser import parse_param_values
from .comparer import compare_param_maps_to_csv
from .auth import require_planyear_user  # ⬅️ NEW
...
@app.get("/debug/list", include_in_schema=False, dependencies=[Depends(require_planyear_user)])  # ⬅️ protected
def debug_list(folder_link: str = Query(...)):
    drive = get_drive()
    ...
    return {
        "service_account": drive._http.credentials.service_account_email,
        "folder_id": folder_id,
        "count": len(res.get("files", [])),
        "files": res.get("files", []),
    }

@app.post("/run", response_model=RunJobResult, dependencies=[Depends(require_planyear_user)])  # ⬅️ protected
async def run_job(
    ...
    # ... (rest of your existing /run implementation remains unchanged)
    try:
        drive = get_drive()
        ...
        for idx, row in enumerate(mapping_rows, start=1):
            ...
            qa_map = parse_param_values(qa_text)
            llm_map = parse_param_values(llm_text)

            csv_bytes, csv_filename = compare_param_maps_to_csv(
                qa_map, llm_map, output_basename=os.path.splitext(os.path.basename(qa_name))[0]
            )

            uploaded = upload_bytes_to_folder_as_file(
                drive, parent_folder_id=output_folder_id, filename=csv_filename, mime_type="text/csv", data=csv_bytes
            )
